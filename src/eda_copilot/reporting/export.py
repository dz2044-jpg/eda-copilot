...
def export_run_artifacts(
    ...
    _write_json(run_dir / "comparison_summary.json", evidence_packet.get("comparison_summary", {}))
    _write_json(run_dir / "visual_specs.json", {"visual_specs": evidence_packet.get("visual_specs", [])})
    _write_csv(run_dir / "data_quality_warnings.csv", evidence_packet.get("data_quality_warnings", []))
    _write_csv(run_dir / "profile_alerts.csv", evidence_packet.get("profile_summary", {}).get("alerts", []))
    _write_csv(run_dir / "quality_checks.csv", evidence_packet.get("quality_checks", {}).get("checks", []))
